/* ------------------------------------------------------------------------------------------------------------------
	...
	Notizen:
    SQL Server führt Views in der Regel als sogenannte „inlined views“ aus, d.h. der 
	View-Text wird in die Abfrage integriert. Das bedeutet, dass der WHERE-Filter 
	(z. B. „WHERE id = 8“) vom Query Optimizer in die rekursive CTE hineingeschoben wird – 
    ...
    Natürlich kann es in komplexen Fällen oder bei speziellen Optimierungsproblemen Ausnahmen 
	geben, aber normalerweise wird der Filter frühzeitig berücksichtigt, sodass nicht unnötig 
	alle Daten berechnet werden müssen.

	--------------------------------------

	Geographic_Units_Range: Tabelle mit Informationen über die Struktur
	Geographic_Units_Rules: Tabelle mit Regeln, wie die Daten verknüpft werden können
	Geographical_Structure: Tabelle mit der Struktur selber

	Check_Geographic_Units_Rules: Funktion überprüft die Struktur  @id AS INTEGER, @Category AS INTEGER
	...
	view_Geo_Structure_Predecessors: View mit Vorgängern, ID ist der Knoten, Level der Abstand, Level = 0 Knoten selber
	view_Geo_Structure_Successors: Nachfolger eines Knoten ID als FollowID, Level = 0 der Knoten sleber, Level sonst Abstand
	view_Geo_Structure_Successors_Leaves: View mit den Blattknoten die zu dem Knoten ID gehören, n´mit LeafID and Level = Distance
...
/* ------------------------------------------------------------------------------------------------------------------
   ...
   !! Achtung, der Name der Straße ist nicht eindeitig
 ------------------------------------------------------------------------------------------------------------------*/
...
/* ------------------------------------------------------------------------------------------------------------------
   ...
   prinzipchell verkehrt, da Parcels einer Unit und einer Straße zugeordnet werden !!!
 ------------------------------------------------------------------------------------------------------------------*/
